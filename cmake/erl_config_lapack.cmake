    ...
    if (ERL_USE_LAPACK)
        ...
        if (ERL_USE_INTEL_MKL)
            ...
            if (NOT TARGET MKL::mkl_core) # suppose MKL is not configured yet.
                ...
                erl_find_package(
                    # We need to find MKL to get MKL_H
                    PACKAGE MKL #
                    REQUIRED #
                    COMMANDS ARCH_LINUX "try `sudo pacman -S intel-oneapi-basekit`" #
                    COMMANDS GENERAL "visit ${_base_toolkit_url}")
                # some packages depending on MKL may use MKL_LIBRARIES, which is set by MKLConfig.cmake MKL_LIBRARIES is
                # a list of library names, not full paths, which will not work correctly without MKL_LIBRARY_DIR
                ...
                # MKL_LIBRARIES contains library names instead of full path. change MKL_LIBRARIES to full paths
                set(_mkl_libs)
            ...
            # the full path of the library. e.g. target_link_libraries(<my_linkable_target> PUBLIC MKL::mkl_core
            # MKL::mkl_sequential MKL::mkl_intel_lp64 PUBLIC MKL::mkl_scalapack_lp64 MKL::mkl_cdft_core
            # MKL::mkl_blacs_intelmpi_lp64 ) target_link_options(<my_linkable_target> PUBLIC MKL::MKL)
