    ...
    if (ERL_BUILD_TEST_${PROJECT_NAME})
        ...
        if (ROS1_ACTIVATED AND CATKIN_ENABLE_TESTING)
            ...
            foreach (file IN LISTS GTEST_SOURCES)
                ...
                erl_target_dependencies(${name} PRIVATE ${${PROJECT_NAME}_TEST_LIBRARIES} GTest::Main
                                        ${${name}_EXTRA_LIBRARIES} ${catkin_LIBRARIES})
                if (${name} IN_LIST ${PROJECT_NAME}_TEST_EXCLUDE_FROM_ALL)
                    ...
                    set(cmd
                        "${_target_path}/${name} --gtest_output=xml:${CATKIN_TEST_RESULTS_DIR}/${PROJECT_NAME}/gtest-${name}.xml"
                    )
                    set(_args
                        "gtest" ${name} "gtest-${name}.xml" #
                        COMMAND ${cmd} #
                        DEPENDENCIES ${name})
        ...
        elseif (ROS2_ACTIVATED)
            file(GLOB ROS2_TEST_SOURCES test/ros2/*.cpp)
            list(APPEND GTEST_SOURCES ${ROS2_TEST_SOURCES})
            foreach (file IN LISTS GTEST_SOURCES)
                get_filename_component(name ${file} NAME_WE)
                add_executable(${name} ${file})
                erl_target_dependencies(${name} PRIVATE ${${PROJECT_NAME}_TEST_LIBRARIES} GTest::Main
                                        ${${name}_EXTRA_LIBRARIES})
                if (${name} IN_LIST ${PROJECT_NAME}_TEST_EXCLUDE_FROM_ALL)
                    message(STATUS "Excluding gtest ${name}")
                    set_target_properties(${name} PROPERTIES EXCLUDE_FROM_ALL TRUE)
                else ()
                    ...
                    ament_add_gtest_test(${name} ${_args})
                    unset(_args)
                    message(STATUS "Adding gtest ${name}")
                endif ()
            endforeach ()
...
function(erl_find_path)
    ...
    find_path(${ERL_OUTPUT} ${ERL_UNPARSED_ARGUMENTS})
...
macro(erl_set_project_paths)
    ...
    set(${PROJECT_NAME}_PYTHON_PKG_DIR ${CMAKE_CURRENT_SOURCE_DIR}/python/${PROJECT_NAME}
        CACHE PATH "Python package directory" FORCE)
    set(${PROJECT_NAME}_PYTHON_BINDING_DIR ${CMAKE_CURRENT_SOURCE_DIR}/python/binding
        CACHE PATH "Python binding directory" FORCE)
    set(${PROJECT_NAME}_SCRIPTS_DIR ${CMAKE_CURRENT_SOURCE_DIR}/scripts CACHE PATH "Script directory" FORCE)
    set(${PROJECT_NAME}_CONFIG_DIR ${CMAKE_CURRENT_SOURCE_DIR}/config CACHE PATH "Config directory" FORCE)
    set(${PROJECT_NAME}_LAUNCH_DIR ${CMAKE_CURRENT_SOURCE_DIR}/launch CACHE PATH "Launch directory" FORCE)
    set(${PROJECT_NAME}_MSG_DIR ${CMAKE_CURRENT_SOURCE_DIR}/msg CACHE PATH "Msg directory" FORCE)
    set(${PROJECT_NAME}_SRV_DIR ${CMAKE_CURRENT_SOURCE_DIR}/srv CACHE PATH "Srv directory" FORCE)
    set(${PROJECT_NAME}_ACTION_DIR ${CMAKE_CURRENT_SOURCE_DIR}/action CACHE PATH "Action directory" FORCE)
    set(${PROJECT_NAME}_DOC_DIR ${CMAKE_CURRENT_SOURCE_DIR}/doc CACHE PATH "Doc directory" FORCE)

    set(${PROJECT_NAME}_BUILD_DIR ${CMAKE_CURRENT_BINARY_DIR} CACHE PATH "Build directory" FORCE)
    set(${PROJECT_NAME}_BUILD_PYTHON_DIR ${CMAKE_CURRENT_BINARY_DIR}/python CACHE PATH "Build python directory" FORCE)
    set(${PROJECT_NAME}_BUILD_PYTHON_PKG_DIR ${CMAKE_CURRENT_BINARY_DIR}/python/${PROJECT_NAME}
        CACHE PATH "Build python package directory" FORCE)
    set(${PROJECT_NAME}_BUILD_TEST_DIR ${CMAKE_CURRENT_BINARY_DIR}/test CACHE PATH "Build test directory" FORCE)
    ...
    elseif (ROS2_ACTIVATED)
        ...
        set(${PROJECT_NAME}_INSTALL_PYTHON_DIR ${Python3_SITELIB}/${PROJECT_NAME}
            CACHE PATH "Path to python directory during installation" FORCE)

        # <INSTALL_PREFIX>/share/PROJECT_NAME
        set(${PROJECT_NAME}_INSTALL_SHARE_DIR ${CMAKE_INSTALL_DATADIR}/${PROJECT_NAME}
            CACHE PATH "Path to share directory during installation" FORCE)
...
macro(erl_setup_compiler)
    ...
    set(CMAKE_CXX_FLAGS_RELEASE "-O3 -funroll-loops -flto -ffat-lto-objects -ffast-math")
...
macro(erl_setup_test)
    ...
    if (ERL_BUILD_TEST)
        ...
        elseif (ROS2_ACTIVATED)
            erl_find_package(PACKAGE GTest REQUIRED COMMANDS UBUNTU_LINUX "try `sudo apt install libgtest-dev`"
...
macro(erl_setup_ros1)
    ...
    erl_find_package(
        ...
        COMMANDS
        ARCH_LINUX
        "try `paru -S python-catkin_tools ros-${ROS_DISTRO}-catkin`")
    ...
    if (${PROJECT_NAME}_MSG_FILES)
        add_message_files(DIRECTORY msg FILES ${${PROJECT_NAME}_MSG_FILES})
        set(should_generate_messages TRUE)
    endif ()
    if (${PROJECT_NAME}_SRV_FILES)
        add_service_files(DIRECTORY srv FILES ${${PROJECT_NAME}_SRV_FILES})
        set(should_generate_messages TRUE)
    endif ()
    if (${PROJECT_NAME}_ACTION_FILES)
        add_action_files(DIRECTORY action FILES ${${PROJECT_NAME}_ACTION_FILES})
        set(should_generate_messages TRUE)
    endif ()
...
macro(erl_setup_ros2)
    ...
    erl_find_package(PACKAGE ament_cmake NO_RECORD REQUIRED #
    ...
    foreach (component IN LISTS ${PROJECT_NAME}_ROS2_COMPONENTS)
        erl_find_package(PACKAGE ${component} REQUIRED #
    ...
    if (interface_files)
        ...
        erl_find_package(PACKAGE rosidl_default_generators NO_RECORD REQUIRED #
...
macro(erl_add_ros_src)
    if (ROS1_ACTIVATED)
        ...
        if (EXISTS ${file})
            ...
            message(WARNING "erl_add_ros_src(): ${file} does not exist")
        endif ()
    elseif (ROS2_ACTIVATED)
        set(file "${CMAKE_CURRENT_LIST_DIR}/src/ros2/ros.cmake")
        if (EXISTS ${file})
            include(${file})
        else ()
            message(WARNING "erl_add_ros_src(): ${file} does not exist")
...
function(erl_target_dependencies target)
    ...
    if (arg_UNPARSED_ARGUMENTS)
        list(APPEND deps ${arg_UNPARSED_ARGUMENTS})
    endif ()
    if (deps)
...
macro(erl_project_setup)
    ...
    foreach (erl_package ${${PROJECT_NAME}_ERL_PACKAGES})
        if (NOT ${erl_package}_FOUND)
            erl_find_package(PACKAGE ${erl_package} REQUIRED #
...
macro(erl_add_pybind_module)
    ...
    if (ERL_BUILD_PYTHON_${PROJECT_NAME})
        ...
        # Use separate rpaths during build and install phases: set(CMAKE_SKIP_BUILD_RPATH  FALSE)

        # Don't use the install-rpath during the build phase: set(CMAKE_BUILD_WITH_INSTALL_RPATH FALSE)
        ...
        # how to check rpath of a library file: objdump -x <library_name>.so | grep 'R.*PATH' output: RUNPATH
        # $ORIGIN:$ORIGIN/lib/<project_name> how to check shared library dependencies of a library file: ldd
        # <library_name>.so
        ...
        # set_target_properties(${${PROJECT_NAME}_PYBIND_MODULE_NAME} PROPERTIES LIBRARY_OUTPUT_DIRECTORY
        # ${${PROJECT_NAME}_PYTHON_DIR}/${PROJECT_NAME})
        if (ROS1_ACTIVATED)
            # copy file to a regular library name so that catkin does not throw an error, but this file may not work
            # with Python3 because the filename is lib<name>.so, which does not match the module name.
            ...
            add_custom_command(
                TARGET ${arg_PYBIND_MODULE_NAME} POST_BUILD
...
macro(erl_install)
    ...
    # ref 1: https://dominikberner.ch/cmake-interface-lib/ (not complete)
    # ref 2: https://gitlab.com/libeigen/eigen/-/blob/master/CMakeLists.txt
    ...
    if (${PROJECT_NAME}_INSTALL_EXECUTABLES)
        ...
        # Export targets defined in this file and define the installation location See
    ...
    if (ROS1_ACTIVATED)
        ...
        if (DEFINED ${PROJECT_NAME}_INSTALL_CATKIN_PYTHON_PROGRAMS)
            catkin_install_python(PROGRAMS ${${PROJECT_NAME}_INSTALL_CATKIN_PYTHON_PROGRAMS} #
    ...
    elseif (ROS2_ACTIVATED)
        ...
        if (DEFINED ${PROJECT_NAME}_BUILD_PYTHON_PKG_DIR AND EXISTS ${${PROJECT_NAME}_PYTHON_PKG_DIR})
